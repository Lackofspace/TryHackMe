...
```
Скрипт `shell.ps1` содержит полезную нагрузку, закодированную в base64, которая предоставляет нам обратную оболочку на порту `4444`. Для её генерации был использован следующий скрипт:
```bash
...
```
Чтобы её поймать, нам необходимо открыть этот порт для прослушивания:
```bash
$ nc -lvnp 4444
